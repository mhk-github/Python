...
def process_cdat_line(line: str, indent: int = 8) -> Optional[DatRecord]:
    ...
    preamble = ' ' * indent
    logger.debug(f"{preamble}Enter process_cdat_line({line=}, {indent=})")
...
def import_cdat_archive(cdat_archive: str, indent: int = 6) -> List[DatRecord]:
    ...
    preamble = ' ' * indent
    logger.debug(
        f"{preamble}Enter import_cdat_archive({cdat_archive=}, {indent=})"
    )

    return_data = []
    with zipfile.ZipFile(cdat_archive, compression=ARCHIVE_TYPE) as archive:
...
def import_cdat(cdat_file: str, indent: int = 6) -> List[DatRecord]:
    ...
    logger = logging.getLogger(__name__)
    preamble = ' ' * indent
    logger.debug(f"{preamble}Enter import_cdat({cdat_file=}, {indent=})")
...
def import_dat(
    ...
    """Imports all DatRecord objects found in a DAT file.
    ...
    preamble = ' ' * indent
    logger.debug(
        f"{preamble}Enter import_dat({dat_file=}, {enc=}, {is_new=}, "
        f"{to_compile=}, {indent=})"
    )
        ...
        for line in in_file:
            ...
            if t:
                if RE_FILE_ATTRIBUTES.match(t):
                    if t.startswith('-----'):
                        logger.warn(f"{preamble}    File attribute '{t}' !")
    ...
    if return_data and to_compile:
        ...
        with zipfile.ZipFile(
            ...
            archive.write(output_file, arcname=PurePath(output_file).name)
            if settings.DELETE_INTERMEDIATE_CDAT_FILES:
                Path(output_file).unlink(missing_ok=True)

    logger.debug(f"{preamble}Leave import_dat(...)")
    return return_data
...
def process_data_file(
    ...
    """Creates DatRecord list from DAT, CDAT and compressed CDAT files.
    ...
    -------
    List[DatRecord]
        A list of DatRecord objects found in the input file
    """

    logger = logging.getLogger(__name__)
    preamble = ' ' * indent
    logger.debug(
        f"{preamble}Enter process_data_file({data_file=}, {enc=}, {is_new=}, "
        f"{to_compile=}, {indent=})"
    )

    return_data = []
    if data_file.endswith(DAT_EXT):
        return_data = import_dat(data_file, enc, is_new, to_compile)
        logger.debug(
            f"{preamble}  Read {len(return_data)} DatRecord objects from "
            f"DAT file '{data_file}'"
        )

    elif data_file.endswith(COMPILED_DAT_EXT):
        return_data = import_cdat(data_file)
        logger.debug(
            f"{preamble}  Read {len(return_data)} DatRecord objects from "
            f"compiled DAT file '{data_file}'"
        )

    elif data_file.endswith(ARCHIVE_EXT):
        return_data = import_cdat_archive(data_file)
        logger.debug(
            f"{preamble}  Read {len(return_data)} DatRecord objects from "
            f"compiled DAT archive file '{data_file}'"
        )

    logger.debug(f"{preamble}Leave process_data_file(...)")
    return return_data
...
def process_dat_globs(
    ...
    preamble = ' ' * indent
    logger.debug(
        f"{preamble}Enter process_dat_globs({dat_globs=}, {enc=}, {is_new=}, "
        f"{to_compile=}, {indent=})"
    )
